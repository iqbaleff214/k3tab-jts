<script setup>
    ...
    const onSearch = (keyword) => {
        Inertia.get(route("customers.index"), { search: keyword }, { preserveState: true, preserveScroll: true });
    }

    const deleteSelected = () => {
        Inertia.delete(route('customers.destroy-bulk', selected.value), {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
                closeDeleteModal();
                        ...
                        <Table :columns="columns" :checkbox="['superadmin', 'supervisor'].includes($page.props.user.role)" :action="['superadmin', 'supervisor'].includes($page.props.user.role)" @selectAllToggle="selectAll" :checkboxState="isSelectedAll">
                            <template #header>
                                <div class="flex items-center justify-between">
                                    <TableSearch placeholder="Number or Name" :search="search" @search="onSearch" />
                            ...
                            <template #tbody>
                                <tr v-for="user in data.data" :key="user" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                    ...
                                    <td class="px-6 py-4 text-right" v-if="['superadmin', 'supervisor'].includes($page.props.user.role)">
                                        <Link class="font-medium text-yellow-600 dark:text-yellow-500 hover:underline" :href="route('customers.edit', user.id)">
                                            Edit
                                        </Link>
                                    </td>
                                </tr>
